...
<?php $news1 = db_first(
  'news',
  'JOIN categories on news.category_id = categories.id order by news.id desc',
  'categories.id as cat_id, categories.name as cat_name, news.id, news.title, news.description, news.image, news.created_at'
);
$news2 = db_first(
  'news',
  'JOIN categories on news.category_id = categories.id where news.category_id!="' . $news1['cat_id'] . '" order by news.id desc',
  'categories.id as cat_id, categories.name as cat_name, news.id, news.title, news.description, news.image, news.created_at'
);
?>
<div class="row mb-2">
    ...
    <div class="col-md-6">
      <div
        ...
        <div class="col p-4 d-flex flex-column position-static">
          ...
          <div class="mb-1 text-body-secondary">{{date('Y-m-d', strtotime($news1['created_at']))}}</div>
          <p class="card-text mb-auto">{{$news1['description']}}</p>
          <a
            href="{{ url('news?category_id='.$news1['cat_id'].'&id='.$news1['id']) }}"
    ...
    <div class="col-md-6">
      <div
        ...
        <div class="col p-4 d-flex flex-column position-static">
          ...
          <div class="mb-1 text-body-secondary">{{date('Y-m-d', strtotime($news2['created_at']))}}</div>
          <p class="card-text mb-auto">{{$news2['description']}}</p>
          <a
            href="{{ url('news?category_id='.$news2['cat_id'].'&id='.$news2['id']) }}"
            class="icon-link gap-1 icon-link-hover stretched-link">
            {{ trans('main.readmore') }}
            <svg class="bi" aria-hidden="true">
              <use xlink:href="#chevron-right"></use>
