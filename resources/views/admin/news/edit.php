...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h2>{{ trans('admin.news') }} - {{ trans('admin.edite')}}</h2>
    <a class="btn btn-info" href="{{ aurl('news')}}" class="btn btn-sm btn-primary">{{ trans('admin.news')}}</a>
</div>
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="form-group">
                ...
                <textarea name="description" placeholder="{{trans('news.description')}}" class="form-control {{!empty(get_error('description'))?'is-invalid': ''}}">{{$news['description']}}</textarea>
            </div>
        </div>
        <div class="col-md-12">
            <div class="form-group">
                <label for="content">{{ trans('news.content') }}</label>
                <textarea name="content" id="content" placeholder="{{trans('news.content')}}" class="form-control {{!empty(get_error('content'))?'is-invalid': ''}}">{{$news['content']}}</textarea>
            </div>
        </div>
    </div>
