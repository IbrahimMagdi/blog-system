...
<body class="d-flex align-items-center py-4 bg-body-tertiary">
  ...
  <main class="form-signin w-100 m-auto">
    ...
    <form method="post" action="{{url(ADMIN.'/do/login')}}">
      ...
      <img
        class="mb-4"
        src="{{url('assets/admin')}}/assets/brand/bootstrap-logo.svg"
        alt=""
        width="72"
        height="57" />
      <h1 class="h3 mb-3 fw-normal">{{ trans('admin.login_page') }}</h1>
      <div class="form-floating">
        <input type="email" name="email" value="<?php echo old('email') ?>" class="form-control <?php echo !empty($email_valid) ? 'is-invalid' : '' ?>" id="floatingInput" placeholder="name@example.com" />
      ...
      <div class="<?php echo !empty($password) ? 'invalid-feedback' : 'valid-feedback' ?>">
        <?php echo $password ?>
      </div>
      <div class="form-check text-start my-3">
        <input class="form-check-input" name="remember-me" type="checkbox" value="remember-me" id="checkDefault" />
        <label class="form-check-label" for="checkDefault">{{ trans('admin.Remember_me') }}</label>
      </div>
    ...
    <?php else: ?>
      <a href="{{url(ADMIN.'/lang?lang=ar')}}" class="nav-link">{{ trans('main.arbic') }}</a>
    <?php endif; ?>
  </main>
  <script
    src="{{url('assets/admin')}}/assets/dist/js/bootstrap.bundle.min.js"
