...
if (!function_exists('render_paginate')) {
    function render_paginate(int $total_pages, $appends): string
    {
        ...
        $p_number = !empty(request('page')) && request('page') <= $total_pages ? request('page') - 1 : 1;
        ...
        $number = !empty(request('page')) && request('page') < $total_pages ? request('page') + 1 : 1;
        $in_disabled = !empty(request('page')) && is_numeric(request('page')) && request('page') > 0 && request('page') == $total_pages ? 'disabled' : '';
        $html .= '<li class="page-item ' . $in_disabled . '">
                    <a class="page-link" href="?'.$request_str . $number . '" aria-label="Next">
...
if (!function_exists('db_paginate')) {
    function db_paginate(string $table, string $query_str, int $limit = 15, string $order_by = 'asc', string $select = '*', array $appends = null): array
    {
