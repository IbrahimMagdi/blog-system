...
if(!function_exists('validation')){
    function validation(array $attributes, array $trans=null, $http_header='redirect', $back=null){
        ...
        foreach($attributes as $attribute=>$rules){
            ...
            $attribute_validte = [];
            $final_atrr = isset($trans[$attribute])? $trans[$attribute] : $attribute;
            foreach(explode('|',$rules) as $rule){
                if($rule == 'email' && !filter_var($value, FILTER_VALIDATE_EMAIL)){
                    $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.email'));
                }elseif($rule == 'required' && (is_null($value) || empty($value) || (isset($value['tmp_name']) && empty($value['tmp_name'])))){
                    $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.required'));  
                }elseif($rule == 'integer' && !filter_var((int) $value, FILTER_VALIDATE_INT)){
                    $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.integer'));  
                }elseif($rule == 'string' && !is_string($value)){
                    $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.string'));  
                }elseif($rule == 'numeric' && !is_numeric($value)){
                    $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.numeric')); 
                }elseif($rule == 'image' && isset($value['tmp_name']) && (!empty($value['tmp_name']) && getimagesize($value['tmp_name']) === false)){
                    $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.image')); 
                }elseif(preg_match('/^in:/i',$rule)){
                    $ex_rule = explode(':', $rule);
                    if(isset($ex_rule[1])){
                        $ex_in = explode(',', $ex_rule[1]);
                        if(!empty($ex_in) && is_array($ex_in) && !in_array($value, $ex_in)){
                            $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.in'));
                        }
                    }
                    ...
                    if(count($ex_rule) > 1 && isset($ex_rule[1])){
                        ...
                        if(!empty($check_unique_db)){
                            $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.unique'));
                        }
                    }
                    ...
                    if(count($ex_rule) > 1 && isset($ex_rule[1])){
                        ...
                        if(empty($check_exists_db)){
                            $attribute_validte[] = str_replace((':attribute'), $final_atrr, trans('validations.exists'));
                        }
                    }
                     
                }
            }
            // bine attribute validation
